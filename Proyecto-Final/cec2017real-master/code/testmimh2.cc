...
void colonization(vector<Solution> &population, int num_childs, double lower, double upper, Random &random){
    ...
    std::uniform_real_distribution<double> distrib_01(0.1, 1.0);
...
template <class Random>
void differentialEvolution(vector<Solution> &population, int iterations, Random &random){
    std::uniform_int_distribution<int> distib_population(0, population.size()-1);
    std::uniform_real_distribution<double> distrib_cr(0.4, 0.6);
    std::uniform_real_distribution<double> distrib_f(0.3, 0.7);
    std::uniform_real_distribution<double> distrib_01(0.0, 1.0);
    
    int dim = population[0].v.size();
    double cr = distrib_cr(random);
    double f = distrib_f(random);
    ...
    for(unsigned iter=0; iter<iterations; ++iter){
        ...
        for(unsigned i=0; i<population.size(); ++i){
            Solution child(dim);
            unsigned parents[3];
            parents[0] = distib_population(random);
            parents[1] = distib_population(random);
            parents[2] = distib_population(random);
            for(unsigned k=0; k<dim; ++k){
                if(distrib_01(random) < cr)
                    child.v[k] = population[parents[0]].v[k]
                                 + f*(population[parents[1]].v[k] - population[parents[2]].v[k]);
                else
                    child.v[k] = population[i].v[k];
            }
            child.fitness = cec17_fitness(&child.v[0]);
 ...
 */
template <class Random>
void mimh2(double &fitness, int max_evals, int dim, double lower, double upper, Random &random){
    ...
    while(evals < max_evals-s_init and s > s_init/3){
        p = ceil(s/fraction);
    ...
    int iters_de = 20;
    while(evals < max_evals-iters_de*population.size()){
        differentialEvolution(population, iters_de, random);
        evals += iters_de*population.size();
        if(VERBOSE){
            cout << "iteration (5*DE): " << endl;
            cout << "\tevals = " << evals << endl;
            int k_aux = 0;
            for(unsigned i_aux=0; i_aux<population.size(); ++i_aux){
                if(population[i_aux].fitness < population[k_aux].fitness)
...
int main(){
    ...
    for(int seed=1; seed<=range_seed; ++seed){
        ...
        for(int funcid=1; funcid<=30; ++funcid){
            ...
            mimh2(fitness, 10000*dim, dim, lower, upper, gen);
