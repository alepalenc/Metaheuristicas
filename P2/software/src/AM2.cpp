...
void AM2(const vector<vector<double> > &matrix, unsigned m, unsigned population_size = 50){
    ...
    vector<unsigned> population_positions;
    ...
    // genero un vector con enteros desde 0 a population.size()
    //   este vector lo usaré para seleccionar aleatoriamente
    //   las soluciones a las que aplicar búsqueda local
    for(unsigned i=0; i<population_size; ++i)
        population_positions.push_back(i);
    ...
    while(evaluations < MAX_EVALUATIONS){
        ...
        random_shuffle(population_positions.begin(), population_positions.end());
        for(unsigned i=0; i<SOLUTIONS_TO_APPLY_LOCAL_SEARCH and evaluations<MAX_EVALUATIONS; ++i){
            evaluations += localSearch(population[population_positions[i]], matrix);
            if(population[population_positions[i]].fitness > population[best_solution].fitness)
